		...
		if (cookies.filter(
			...
			getUser(function(response) {
				if(response.success) {
					...
					document.cookie = 'graphjs_' + key + '_id=' + response.id + '; path=/; expires=' + expiry.toGMTString() + '; samesite=none';;
					document.cookie = 'graphjs_' + key + '_username=' + response.username + '; path=/; expires=' + expiry.toGMTString() + '; samesite=none';;
				} else {
					document.cookie = 'graphjs_' + key + '_session_off=true; path=/; samesite=none';
				}
				callback(response);
			});
		}
