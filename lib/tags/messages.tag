<graphjs-messages class={'graphjs-element graphjs-root ' + boxStyle} style={
    ...
    <div class="graphjs-header" if={!hideTitle}>
        <a class="graphjs-option graphjs-left" onclick={handleNewMessage} if={loaded&&opts.recipientDiscovery!="list"}>
    ...
    <script>
        ...
        this.listAllUsers = (opts.recipientDiscovery == "list")
        ...
        this.handleList = (messages) => {
            ...
            if(self.listAllUsers) {
                self.handlePossiblePartners(true);
            }
            else {
                self.loaded = true;
                self.update();
        ...
        this.handleNewMessage = (event) => {
            ...
            this.handlePossiblePartners(false);
            this.update();
        }
        this.handlePossiblePartners = (popList) => {
            let self = this;
            Object.keys(self.possiblePartners).length > 0 || getMembers(function(response) {
                if(response.success) {
                    ...
                    if(popList) {
                        for(let possiblePartner of self.possiblePartners) {
                            if(self.list[possiblePartner.id]==undefined) {
                                if(possiblePartner.id!=self.userId && !self.partners.includes(possiblePartner.id))
                                    self.partners.push(possiblePartner.id);
                                let item = {
                                        partner: possiblePartner.id,
                                        avatar: possiblePartner.avatar,
                                        username: possiblePartner.username,
                                        message: "",
                                        is_read: true
                                };
                                self.list[(possiblePartner.id)] = item;
                            }
                        }
                        self.loaded = true;
                        self.update();
