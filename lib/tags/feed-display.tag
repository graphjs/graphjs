<graphjs-feed-display class="graphjs-root" style={
    ...
    <div class="graphjs-content">
        <div class="graphjs-information">
            <div class="graphjs-credit" if={profile}>
                <img data-link="profile" data-id={profile.id} onclick={handleShow} src={profile.avatar ? downsizeImage(profile.avatar, 50) : 'https://res.cloudinary.com/graphjs/image/upload/graphjs/content/avatars/user.png'} />
                <span>
                    <b data-link="profile" data-id={profile.id} onclick={handleShow}>{profile.username || 'Unknown User'}</b>
                    <time data-timestamp={createTime}>{handleTime(createTime)}</time>
                </span>
            </div>
    ...
    <style type="less">
        ...
        @import '../styles/components/feed-display.less';
        @prefix: graphjs;

        @{prefix}-feed-display.@{prefix}-root {
            display: block;
            z-index: 10;
            position: fixed;
            top: 0;
            ...
            .@{prefix}-content {
                .@{prefix}-information {
                    ...
                    a {
                        ...
                        &:hover {
                            background-color: fade(@default-color, 25%);
                        }
                        svg {
                            width: 20%;
                            margin: 40%;
                    ...
                    .@{prefix}-container {
                        ...
                        .@{prefix}-media {
                            ...
                            & > a {
                                ...
                                height: 3em;
                                svg {
                                    .icon-color-states(tint(@default-color, 10%));
                                }
                            }
                        }
                    }
    ...
    <script>
        ...
        this.handleContent = (callback) => {
            let item = window.GraphJSMedia[this.id];
            this.owner = item.owner;
            this.type = item.type;
            this.createTime = item.timestamp;
            this.media = item[item.type];
            this.handleInformation(this.owner);
        }
