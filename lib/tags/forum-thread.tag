<graphjs-forum-thread
    class={'graphjs-root graphjs-composer' + (opts.minor ? '' : ' graphjs-box')}
    style={
...
>
    <div class="graphjs-header">
        ...
        <div if={window.innerWidth < 768} class="graphjs-title">{opts.title || 'Thread by ' + (currentAuthor || 'Forum User')}</div>
        <div if={window.innerWidth >= 768} class="graphjs-title">{title || 'Thread by ' + (currentAuthor || 'Forum User')}</div>
    </div>
    ...
    <a class="graphjs-promo graphjs-bottom graphjs-left" href="http://graphjs.com" target="_blank">
