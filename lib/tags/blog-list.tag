...
>
    <div class={'graphjs-content' + (loaded ? '' : ' graphjs-loading') + (blocked ? ' graphjs-blocked' : '') + (matchedPosts.length > pageLimit ? ' graphjs-pagination' : '')}>
        <div class="graphjs-list" if={loaded}>
            <a each={matchedPost, index in matchedPosts} class={'graphjs-item' + ((index + 1 > (page - 1) * pageLimit && index + 1 <= Math.min(matchedPosts.length, page * pageLimit)) ? '' : ' graphjs-hidden')} data-link="post" data-id={matchedPost} onclick={opts.minor ? handleCallback : handleShow}>
                <div class="graphjs-title">
                    {postsData[matchedPost] && postsData[matchedPost].title}
                </div>
                <time data-time={postsData[matchedPost] && postsData[matchedPost].timestamp}>
                    {postsData[matchedPost] && handleTime(postsData[matchedPost].timestamp)}
                </time>
                <img data-link="profile" data-id={postsData[matchedPost].author} onclick={handleShow} />
            </a>
            <div class="graphjs-placeholder graphjs-item" if={matchedPosts.length <= 0}>
    ...
    <script>
        ...
        this.handleContent = () => {
            ...
            getBlogPosts(function(response) {
                if(response.success) {
                    for(let post of response.blogs) {
