<graphjs-profile-card class={'card box' + (loaded ? '' : ' loading')}>
    <a onclick={following ? handleUnfollow : handleFollow} class="left option" if={profile && userId && userId != id}>
    ...
    <a class="right option" onclick={handleMessageComposer} if={profile && userId && userId != id}>
