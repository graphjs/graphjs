...
import './docs/components/feedback.tag';
...
const init = function(id, settings) {
	...
	window.GraphJSConfig['host'] = 'http://phonetworks.com';
	if(settings) {
		window.GraphJSConfig['host'] = settings.host || 'http://phonetworks.com';
		if(settings.theme) {
			window.GraphJSConfig['theme'] = settings.theme;
			document.body.className = document.body.className.replace(/\bgraphjs-theme\S+/ig, '');
			document.body.classList.add('graphjs-theme-' + settings.theme);
		}
		if(settings.color) {
			window.GraphJSConfig['color'] = settings.color;
			renderColor(settings.color);
		}
	}
	riot.mount('*');
}
