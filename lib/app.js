...
let requiredModules = [];
// const requiredModules = REQUIRED_MODULES.split(",");
if(REQUIRED_MODULES === "all"){
	requiredModules = [ "general", "activity", "authentication", "blog", "comments", "feed",
						"forum", "groups", "members", "messages", "show", "star", "privatecontent", "stripe"
					  ];
} else if(REQUIRED_MODULES === "groupsv2"){
	requiredModules = [ "authentication", "blog",  "messages"]
} else {
	requiredModules = REQUIRED_MODULES.split(",");
}
...
if(requiredModules.includes("authentication")) {
	(async () => {
		await import('./tags/auth.tag');
		await import('./tags/auth-state.tag');
		await import('./tags/auth-login.tag');
		await import('./tags/auth-register.tag');
		await import('./tags/auth-reset.tag');
	})()
}
...
// General
// if(requiredModules.includes("general")){
  //import('./scripts/getId.js').then(getId => window.GraphJS.getId = getId);
  (async () => {
    // General
    const getId = requiredModules.includes("general") ? await import('./scripts/getId.js') : null;
    // Activity
    const getActivityToken = requiredModules.includes("activiy") ? await import('./scripts/getActivityToken.js') : null;
	  
	// Authentication
	const getSession = requiredModules.includes("authentication") ? await import('./scripts/getSession.js') : null;
	const getUser = requiredModules.includes("authentication") ? await import('./scripts/getUser.js') : null;
	const login = requiredModules.includes("authentication") ? await import('./scripts/login.js') : null;
	const logout = requiredModules.includes("authentication") ? await import('./scripts/logout.js') : null;
	const register = requiredModules.includes("authentication") ? await import('./scripts/register.js') : null;
	const reset = requiredModules.includes("authentication") ? await import('./scripts/reset.js') : null;
	const verify = requiredModules.includes("authentication") ? await import('./scripts/verify.js') : null;
	
	// Blog
	const commentBlogPost = requiredModules.includes("blog") ? await import('./scripts/commentBlogPost.js') : null;
	const editBlogPost = requiredModules.includes("blog") ? await import('./scripts/editBlogPost.js') : null;
	const editBlogComment = requiredModules.includes("blog") ? await import('./scripts/editBlogComment.js') : null;
	const getBlogComments = requiredModules.includes("blog") ? await import('./scripts/getBlogComments.js') : null;
	const getBlogPost = requiredModules.includes("blog") ? await import('./scripts/getBlogPost.js') : null;
	const getBlogPosts = requiredModules.includes("blog") ? await import('./scripts/getBlogPosts.js') : null;
	const publishBlogPost = requiredModules.includes("blog") ? await import('./scripts/publishBlogPost.js') : null;
	const removeBlogPost = requiredModules.includes("blog") ? await import('./scripts/removeBlogPost.js') : null;
	const removeBlogComment = requiredModules.includes("blog") ? await import('./scripts/removeBlogComment.js') : null;
	const startBlogPost = requiredModules.includes("blog") ? await import('./scripts/startBlogPost.js') : null;
	const unpublishBlogPost = requiredModules.includes("blog") ? await import('./scripts/unpublishBlogPost.js') : null;
	...
	window.GraphJS = {
		...
		editBlogPost, editBlogComment, getBlogComments, getBlogPost, getBlogPosts, removeBlogPost, removeBlogComment, commentBlogPost, startBlogPost, publishBlogPost, unpublishBlogPost,// Blog
