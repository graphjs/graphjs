module.exports = {
    ...
    decode: function (str) {
        ...
        var regex = /\[metadata\]\(type=(?<type>[a-zA-Z0-9_-]*?);key=\{(?<key>[^\\{}]*(?:\\[^][^\\{}]*)*)\};value=\{(?<value>[^\\{}]*(?:\\[^][^\\{}]*)*)\}\)/g;
        ...
        while ((matches = regex.exec(str)) !== null) {
            metadata.push({
                ...
                key: matches.groups.key.replace(/\\\{/g, '{').replace(/\\\}/g, '}'),
                value: matches.groups.value.replace(/\\\{/g, '{').replace(/\\\}/g, '}'),
            });
        }
    ...
    encode: function (parsed) {
        ...
        var metastr = metadata.map(function (meta) {
            var type = meta.type;
            var key = meta.key.replace(/\{/g, '\\{').replace(/\}/g, '\\}');
            var value = meta.value.replace(/\{/g, '\\{').replace(/\}/g, '\\}');
